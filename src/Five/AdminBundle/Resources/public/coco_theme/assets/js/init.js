...
$( ".portlets" ).sortable({
    ...
    //placeholder: "picture-box-placeholder",  
    start: function(e, ui){      
        //ui.placeholder.append(ui.item.html());
        ui.placeholder.height(ui.item.outerHeight());
        //ui.placeholder.width(248);
/*        ui.helper.height(ui.item.height());
        ui.helper.width(ui.item.width());        
        ui.placeholder.css('overflow', 'hidden');
        ui.placeholder.css( 'width', $('.picture-box:first').width() + 'px' );
        ui.placeholder.css( 'height', $('.picture-box:first').height() + 'px' );*/
        //ui.helper.css('margin-bottom', '25px');
        console.log( ui.helper[0].className );
        ui.helper[0].className = 'picture-box ui-sortable-helper';
        //ui.item[0].className = 'picture-box';
//picture-box col-lg-3 col-md-6 col-sm-6 col-xs-12
        console.log( ui.item[0].className, ui.item[0].style );
        //ui.item.css({'display':'block', 'visibility':'hidden'});

    },    
    receive: function(event, ui) {$("body").trigger("resize")}
});
//$( ".portlets" ).sortable( "option", "grid", [ 20, 10 ] );
...
$(".iswitch").each(function(){
    $( this ).click( function()
    {
        ...
        var container = $( this ).parent( '.publish_action' );
        var spanLabel = container.find( '.publish_action_label:first' );
        var labelInnerHtml = spanLabel.html();
        var inputPublish   = container.find( 'input[data-publish]:first' );
        var isPublished    = inputPublish.prop( 'checked' );
        var url = inputPublish.data( 'publish' );


        //console.log( 'spanLabel -->', spanLabel );
        //console.log(inputPublish.data( 'publish' ).replace( 'false', 'true' ));
        //$( this ).parent().find( '.publish_action:first' );

        if($( this ).hasClass( 'on' ) )          
        {
          
          
            inputPublish.data( 'publish', inputPublish.data( 'publish' ).replace( 'false', 'true' ) );
            labelInnerHtml = labelInnerHtml.replace( 'Private', 'Public' );
            spanLabel.removeClass( 'text-private' ).addClass( 'text-public' ).html( labelInnerHtml );
            //console.log('This is checked');
        }
        else
        {
            url = url.replace('false', 'true');
            inputPublish.data( 'publish', inputPublish.data( 'publish' ).replace( 'true', 'false' ) );
            labelInnerHtml = labelInnerHtml.replace( 'Public', 'Private' );
            spanLabel.removeClass( 'text-public' ).addClass( 'text-private' ).html( labelInnerHtml );
            //console.log('This is OFF');
        }


          url = url.replace('false', inputPublish.prop('checked'));
          url = url.replace('true', inputPublish.prop('checked'));
        $.post( url, function( resp )
        {
          //console.log( 'Updated Published ->', resp, isPublished );
        } );
        console.log( url,inputPublish.prop('checked') );
    } ); 
  } );
...
/*    function()
    {
      console.log('this is changed', $( this ).prop( "checked" ));
    } )*/
});
